...
// build array maps of the form inputs & control groups //
	...
	this.formFields = [$('#title-tf'), $('#desc-tf'), $('#url-tf'), $('#order-tf'),$('#sta-tf')];
	this.controlGroups = [$('#title-cg'), $('#desc-cg'), $('#url-cg'), $('#order-cg'),$('#sta-cg')];
...
// bind the form-error modal window to this controller to display any errors //
	...
	this.validatetextarea = function(s)
	{
...
AccountValidator.prototype.validateForm = function()
{
	...
	if (this.validateName(this.formFields[0].val()) == false) {
		this.controlGroups[0].addClass('error'); e.push('Url is a required field');
		//this.showColor();
	}
	if (this.validatetextarea(this.formFields[1].val()) == false) {
		this.controlGroups[1].addClass('error'); e.push('Title is a required field');
		//this.showColor();
	}
